...
const Loader = ({ setIsLoaded, loadingPercentage }) => {
  ...
  useEffect(() => {
    if (typeof loadingPercentage !== 'number' || loadingPercentage < 0 || loadingPercentage > 100) {
      console.error("Invalid loadingPercentage value");
      return;
    }
  ...
  useEffect(() => {
    ...
    if (loadingPercentage === 100) {
      const timeline = gsap.timeline({ onComplete: () => setIsLoaded(true) });
      timeline
        .to("#loaderlogo", {
          delay: 1,
          rotate: 90,
  ...
  return (
    <div id="loaderoverlay" className="loaderoverlay relative w-screen h-screen flex justify-center items-center overflow-hidden">
      ...
      <p ref={loaderTextRef} className="mix-blend-difference absolute bottom-5 left-5 text-7xl text-white font-outfit">
        ...{loadingPercentage}
      </p>
    </div>
  );
